...
#        self.yr = np.zeros_like(self.t)
    ...
    def plotWay(self, figNo=1, xmin=-10, xmax=10, ymin=-10, ymax=10, mod=None, title = 'robot0'):
        # -------------------------------------------------------------------------
...
#            graph.plot(self.state[0,-1], self.state[1,-1], color = 'r', marker='o')
    ...
    def plotAnswer(self, figNo=1, xmin=-10, xmax=10, ymin=-10, ymax=10, mod=None, title = 'robot0'):
...
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    def plot(self, figNo=1, xmin=-10, xmax=10, ymin=-10, ymax=10, mod=None):
        # -------------------------------------------------------------------------
        startNo=figNo+self.nbOfRobots
        
        for i in range(0, self.nbOfRobots):
            title = "robot["+str(i)+"]"
            self.robotSimulation[i].plotWay(figNo+i, xmin, xmax, ymin, ymax, mod, title)
            self.robotSimulation[i].plotAnswer(startNo, xmin, xmax, ymin, ymax, mod, title)
            startNo+=2
...
#            graph.plot(self.state[0,-1], self.state[1,-1], color = 'r', marker='o')
    ...
    def plotFleetFinal(self, figNo=1, xmin=-10, xmax=10, ymin=-10, ymax=10, mod=None, links=True):
        # -------------------------------------------------------------------------
...
#            graph.plot(self.state[0,-1], self.state[1,-1], color = 'r', marker='o')
        ...
        for i in range(0, self.nbOfRobots):
            for j in range(0, self.nbOfRobots):
                xi = self.robotSimulation[i].state[0, int(self.t.shape[0])-1]
                yi = self.robotSimulation[i].state[1, int(self.t.shape[0])-1]
                xj = self.robotSimulation[j].state[0, int(self.t.shape[0])-1]
                yj = self.robotSimulation[j].state[1, int(self.t.shape[0])-1]
                if (links == True):
                    graph.plot([xi, xj], [yi, yj], color='grey',
                               alpha=0.3, linestyle='--')
            graph.plot(xi, yi, marker='8', linestyle="None", markersize=10)
